...
void MainForm::init()
{
   ...
   zfthread -> sigConnect(this, SLOT(updateCurrentZF()));
    ...
    videoWidget -> setCursor(QCursor(pointingHandCursor));  // turn on default point-and-click centering mode
...
{    
 ...
 long reg = 0;
 int i = zfthread -> connectZoomFocus((char*)DataSource :: zfmotorIP.ascii(), DataSource :: zfmotorPort);
 if(i != 0)
 {
...
//
//   move pin (x,y) microns from current position
//
...
void MainForm :: clickAndMoveMotors(float x, float y)
{
    ...
    printf("sending %f to pinx.\n", pinx / 1000.0);
    printf("sending %f to piny.\n", piny / 1000.0);
...
void MainForm::rotateNegativeFiveButton()
{
    QTextStream os(socket);
    os << "MPHI -5 \nend_of_command\n";
    phiAngle -= 5.0;
...
void MainForm::rotateFiveButton()
{
    QTextStream os(socket);
    os << "MPHI 5 \nend_of_command\n";
    phiAngle += 5.0;
...
void MainForm::rotateFortyFiveButton()
{
    QTextStream os(socket);
    os << "MPHI 45 \nend_of_command\n";
    phiAngle += 45.0;
...
void MainForm::rotateNegativeFortyFiveButton()
{
    QTextStream os(socket);
    os << "MPHI -45 \nend_of_command\n";
    phiAngle -= 45.0;
